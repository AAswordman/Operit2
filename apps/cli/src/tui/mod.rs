...
async fn run_link_tui_command(args: &[String]) -> Result<(), String> {
    let session_name = args
        .get(0)
        .ok_or_else(|| "usage: operit2 cli link tui <session> [--chat <chat-id>]".to_string())?
        .clone();
...
pub(crate) async fn run_link_command(args: &[String]) -> Result<(), String> {
    match args.first().map(String::as_str) {
        ...
        Some("tui") => run_link_tui_command(&args[1..]).await,
        _ => {
            print_link_usage();
            Ok(())
        }
    ...
    while index < args.len() {
        match args[index].as_str() {
            "--bind" => {
                ...
                bind_address = args
                    .get(index)
                    .ok_or_else(|| "usage: operit2 cli link serve [--bind <addr:port>] [--token <token>]".to_string())?
                    .clone();
            }
            "--token" => {
                index += 1;
            ...
            _ => {
                return Err("usage: operit2 cli link serve [--bind <addr:port>] [--token <token>]".to_string());
            }
        }
        index += 1;
    }
...
async fn run_link_hello_command(args: &[String]) -> Result<(), String> {
    let (url, token) = parse_remote_url_token(args, "usage: operit2 cli link hello <url> --token <token>")?;
...
async fn run_link_connect_command(args: &[String]) -> Result<(), String> {
    let (url, token, save_name) =
        parse_remote_url_token_save(args, "usage: operit2 cli link connect <url> --token <token> [--save <name>]")?;
    let client = RemoteLinkClient::new(url);
    let hello = client.hello(&token).await?;
    println!(
...
async fn run_link_call_command(args: &[String]) -> Result<(), String> {
    let name = args
        .get(0)
        .ok_or_else(|| "usage: operit2 cli link call <session> <target-path> <method-name> [args-json]".to_string())?;
...
async fn run_link_watch_command(args: &[String]) -> Result<(), String> {
    let name = args
        .get(0)
        .ok_or_else(|| "usage: operit2 cli link watch <session> <target-path> <property-name> [args-json]".to_string())?;
    let target_path = args
        .get(1)
        .ok_or_else(|| "usage: operit2 cli link watch <session> <target-path> <property-name> [args-json]".to_string())?;
...
fn merge_sync_operations(
    ...
    for value in sync_operation_array(left)?.into_iter().chain(sync_operation_array(right)?) {
        let opId = value
            .get("opId")
            .and_then(|value| value.as_str())
