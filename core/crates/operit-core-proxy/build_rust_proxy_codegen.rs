...
pub(crate) fn render_generated_proxy(objects: &[SourceObject]) -> String {
    ...
    let application_id = objects
        ...
        .object_id;
    output.push_str(&format!("        let response = self.client.call(operit_link::CoreCallRequest::new(generated_proxy_request_id(), {application_id}, \"runCoreCommand\", args)).await;\n"));
    ...
    for object in objects {
        ...
        if object.has_proxy_value_call_methods() {
            ...
            output.push_str("        let response = self.client.call(operit_link::CoreCallRequest::new(generated_proxy_request_id(), self.target_object_id, methodName, self.generatedArgs(args))).await;\n");
            output.push_str("        let value = response.result?;\n");
            output.push_str("        operit_link::fromCoreValue(value).map_err(|error| operit_link::CoreLinkError::new(\"INVALID_RESPONSE\", error.to_string()))\n");
            output.push_str("    }\n\n");
        }
        ...
        if object.has_proxy_snapshot_watch_methods() {
            ...
            output.push_str("        let event = self.client.watchSnapshot(operit_link::CoreWatchRequest::new(generated_proxy_request_id(), self.target_object_id, propertyName, self.generatedArgs(args))).await?;\n");
...
fn render_proxy_reverse_stream_method(method: &SourceMethod) -> String {
    ...
    output.push_str(&format!(
        "        let request = operit_link::CorePushRequest::new(generated_proxy_request_id(), self.target_object_id, {:?}).withArgs(self.generatedArgs({}));\n",
        method.name, args
    ));
    ...
    match &watch.stream {
        ...
        | WatchStreamProtocol::StringStream => {
            ...
            let method_code = format!(
                "    pub async fn {}(&mut self{}) -> Result<operit_link::CoreEventStream, operit_link::CoreLinkError> {{\n        self.client.watch(operit_link::CoreWatchRequest::new(generated_proxy_request_id(), self.target_object_id, {:?}, self.generatedArgs({}))).await\n    }}\n\n",
                method.name, params, method.name, args_json
            );
            render_cfg_attrs(method) + &render_doc_comments(method) + &method_code
        }
