...
use quote::ToTokens;
use syn::{Expr, ImplItem, Item, Meta, MetaNameValue, ReturnType};
...
fn main() {
    ...
    let runtime_root = manifest_dir.join("../operit-runtime/src");
...
fn render_route_catalog(declarations: &BTreeSet<(String, String, String, String)>) -> String {
    ...
    output.push_str("pub fn generated_space_call_route(request: &operit_link::CoreCallRequest) -> Option<GeneratedSpaceRoute> { generated_space_route_for_id(request.targetObjectId, &request.methodName) }\n\n");
    output.push_str("/// Resolves one annotation-generated Space route from a standard Link watch request.\n");
    output.push_str("pub fn generated_space_watch_route(request: &operit_link::CoreWatchRequest) -> Option<GeneratedSpaceRoute> { generated_space_route_for_id(request.targetObjectId, &request.propertyName) }\n\n");
    output.push_str("/// Resolves one annotation-generated Space route from a standard Link push request.\n");
    output.push_str("pub fn generated_space_push_route(request: &operit_link::CorePushRequest) -> Option<GeneratedSpaceRoute> { generated_space_route_for_id(request.targetObjectId, &request.methodName) }\n\n");
