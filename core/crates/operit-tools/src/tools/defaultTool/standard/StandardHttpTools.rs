...
/// Operations supported by the standard HTTP tool executor.
...
/// Dispatches HTTP tool calls through host networking services.
...
impl StandardHttpTools {
    ...
    /// Sends a standard HTTP request through the host.
    ...
    #[allow(non_snake_case)]
    /// Reads, writes, or clears cookies for a domain.
    ...
    #[allow(non_snake_case)]
    /// Sends a multipart HTTP request with form fields and file parts.
...
impl ToolExecutor for HttpToolExecutor {
    ...
    fn accessSpec(&self, tool: &AITool) -> Result<ToolAccessSpec, String> {
        let effect = match self.operation {
            HttpToolOperation::HttpRequest => httpMethodEffect(
                optionalParameterValue(tool, "method")
                    .unwrap_or_else(|| "GET".to_string())
                    .as_str(),
            ),
...
fn httpMethodEffect(method: &str) -> ToolEffect {
