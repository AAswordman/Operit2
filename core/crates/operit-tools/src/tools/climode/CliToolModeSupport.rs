...
- 不要直接调用隐藏工具。先用 `search`，再用发现到的目标工具名和 JSON 参数调用 `proxy`。"#
        ...
        for category in categories {
            for tool in category.tools {
                ...
                let entry = HiddenToolCatalogEntry {
                    ...
                    keywords: vec![category.categoryName.clone()],
    ...
    #[allow(non_snake_case)]
    fn buildParameterHints(tool: &ToolPrompt) -> Vec<String> {
        if let Some(parametersStructured) = tool
            .parametersStructured
            .as_ref()
            .filter(|parametersStructured| !parametersStructured.is_empty())
        {
            return parametersStructured
                .iter()
                .map(|parameter| {
