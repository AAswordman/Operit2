...
use crate::javascript::JsExecutionResultProtocol::{extractJsExecutionFailure, JsExecutionFailure};
...
mod tests {
    ...
    use operit_host_api::{
        AppListData, AppOperationData, AppUsageTimeResultData, DeviceInfoData, HostError,
        HostResult, LocationData, NotificationData, OCRLanguage, OCRQuality, RuntimeStorageEntry,
        RuntimeStorageHost, SystemOperationHost, SystemSettingData,
    };
    ...
    use crate::core::tools::AIToolHandler::AIToolHandler;
    use operit_tools::ToolPackage::{PackageTool, ToolPackage};
    ...
    struct TestSystemOperationHost;

    impl TestSystemOperationHost {
        fn unused<T>() -> HostResult<T> {
            Err(HostError::new("test system operation is not used"))
        }
    }

    impl SystemOperationHost for TestSystemOperationHost {
        fn getSystemLanguageCode(&self) -> HostResult<String> {
            Ok("en".to_string())
        }

        fn toast(&self, _message: &str) -> HostResult<()> {
            Self::unused()
        }

        fn sendNotification(&self, _title: &str, _message: &str) -> HostResult<()> {
            Self::unused()
        }
        ...
        fn installApp(&self, _path: &str) -> HostResult<AppOperationData> {
            Self::unused()
        }

        fn uninstallApp(&self, _packageName: &str) -> HostResult<AppOperationData> {
            Self::unused()
        }

        fn listInstalledApps(&self, _includeSystemApps: bool) -> HostResult<AppListData> {
            Self::unused()
        }

        fn startApp(&self, _packageName: &str) -> HostResult<AppOperationData> {
            Self::unused()
        }

        fn stopApp(&self, _packageName: &str) -> HostResult<AppOperationData> {
            Self::unused()
        }
        ...
        fn getNotifications(
            &self,
            _limit: i32,
            _includeOngoing: bool,
        ) -> HostResult<NotificationData> {
            Self::unused()
        }
    ...
    fn test_paths(name: &str) -> RuntimeStorePaths {
        let root = std::env::temp_dir().join(format!(
            "toolpkg-runtime-sdk-tests-{}-{name}",
            std::process::id()
        ));
    ...
    fn test_context() -> OperitApplicationContext {
        let mut context = OperitApplicationContext::new();
        context.systemOperationHost = Some(Arc::new(TestSystemOperationHost));
        context
    }
    ...
    fn toolpkg_manager(script: &str) -> (JsToolManager, Arc<Mutex<PackageManager>>) {
        ...
        let manager = JsToolManager {
            ...
            toolHandler: AIToolHandler::withContext(context),
