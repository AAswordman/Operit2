...
impl TerminalHost for AndroidTerminalHost {
    ...
    fn executeInSession(
        ...
        let result = executeAndroidPtyCommandInSession(
            commandOutput.clone(),
            &normalizedCommand,
            timeoutMs,
        )?;
        // Waiting for the shell to recover must not block operations on other sessions.
        let workingDir = if result.timedOut {
            cancelTimedOutAndroidPtyCommand(&writer, &commandOutput)?
        } else {
            result.workingDir.clone()
        };
        {
            let mut state = self.lockState()?;
...
fn cancelTimedOutAndroidPtyCommand(
    writer: &Arc<Mutex<RawFd>>,
    output: &AndroidPtyCommandOutput,
) -> HostResult<Option<String>> {
    writeAndroidPtyBytes(writer, b"\x03")?;
