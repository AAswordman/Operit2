use js_sys::Object;
use operit_host_api::{
    HostResult, TtsPlaybackHost, TtsPlaybackRequest, TtsPlaybackStatus,
};
use wasm_bindgen::prelude::*;

use crate::common::{
    call_tts_playback, read_bool_property, read_string_property, set_property,
};
...
impl TtsPlaybackHost for WebTtsPlaybackHost {
    fn speakText(&self, request: TtsPlaybackRequest) -> HostResult<TtsPlaybackStatus> {
        tts_status(call_tts_playback("speakText", &[tts_request_to_js(request)])?)
    }
