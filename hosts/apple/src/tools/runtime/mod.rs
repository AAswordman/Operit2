...
impl ManagedRuntimeHost for AppleManagedRuntimeHost {
    ...
    fn startRuntimeProcess(
        ...
        let stdin = match child.stdin.take() {
            Some(handle) => handle,
            None => return Err(HostError::new("managed runtime process has no stdin")),
        };
        let stdout = match child.stdout.take() {
            Some(handle) => handle,
            None => return Err(HostError::new("managed runtime process has no stdout")),
        };
        let stderr = match child.stderr.take() {
            Some(handle) => handle,
            None => return Err(HostError::new("managed runtime process has no stderr")),
        };
...
fn ensureExecutablePath(path: &str) -> HostResult<String> {
    ...
    match findExecutable(&[path]) {
        Some(executable) => Ok(executable),
        None => Err(HostError::new(format!("Executable not found: {path}"))),
    }
}
