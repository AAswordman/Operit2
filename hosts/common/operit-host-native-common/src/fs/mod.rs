...
impl FileSystemHost for PosixFileSystemHost {
    ...
    fn validatePath(&self, path: &str, paramName: &str) -> HostResult<()> {
        ...
        if !pathValue.is_absolute() {
            return Err(HostError::new(format!(
                "Invalid path: '{path}'. Path must be an absolute POSIX path."
            )));
        }
        Ok(())
    }
    ...
    fn openFile(&self, path: &str) -> HostResult<()> {
        Err(HostError::new(format!(
            "Shared POSIX file host does not own platform open_file: {path}"
        )))
    }

    fn shareFile(&self, path: &str, title: &str) -> HostResult<()> {
        Err(HostError::new(format!(
            "Shared POSIX file host does not own platform share_file: {path} ({title})"
        )))
    }
}
...
fn validate_readable_file(host: &PosixFileSystemHost, path: &str) -> HostResult<()> {
