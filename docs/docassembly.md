...
host capability
  ...
  -> hosts/{android,apple,linux,ohos,web,windows}
```
...
TtsPlaybackService 只调用 TtsPlaybackHost 播放生成音频或控制系统 TTS
Flutter chat UI 只发起生成/播放请求、展示悬浮控制面板
系统 TTS 不在 Flutter UI 中按平台分叉实现
生成音频不在 Flutter UI 中解码或直接调用平台播放器
...
Apple    -> RuntimeHostInteractionService 请求 owner app，AVSpeechSynthesizer 负责系统语音，AVAudioPlayer 播放生成音频
OHOS     -> RuntimeHostInteractionService 请求 owner app，独立 AVPlayer 播放生成音频；API 18 不声明 SYSTEM_TTS
```
